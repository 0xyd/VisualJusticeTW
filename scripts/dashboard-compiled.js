			...
			indexDB: [{
				...
				content: {
					data: [{
						name: '本年執行人數',
						topic: ['總數'],
						availableChartTypes: ['年度總量', '走勢', '面積圖']
					}, {
						name: '本年入監人數',
						topic: ['總數'],
						availableChartTypes: ['年度總量', '走勢', '面積圖']
	...
	chartRefresh: function chartRefresh() {
		...
		if (menuIndex === 0) {
			...
			cDatasetIdx = findCurrentDatasetIndex(this.state),
			    r = this.state.indexDB[cDatasetIdx].content.data.find(function (d) {
			...
			var cDatasetIdx = findCurrentDatasetIndex(this.state),
			    ...
			    r = this.state.indexDB[cDatasetIdx].content.data[cTopicIdx].availableChartTypes.find(function (d) {
		...
		function findCurrentTopicIndex(s, datasetIdx) {
			...
			var index = s.indexDB[datasetIdx].content.data.findIndex(function (d) {
		...
		if (this.props.dataset !== nextProps.dataset && nextStates.sheetName === this.state.dataSheets[0].name) {
			...
			// update the current viewing of dataSheet[0] (監獄)
					...
					(function () {
						...
						if (nextProps.chartType !== '年度總量') _this.props.barGraph.bePhantom();
						...
						_this.props.barGraph.update(nextStates.sheetUrl, _this.state.chartAxes.xAxis, _this.state.chartAxes.yAxis, nextStates.dataTopic).then(function (jsonOutput) {
							...
							lG.plotBars(jsonOutput.data, jsonOutput.pad, jsonOutput.updatedBars, jsonOutput.barWidth / 2).then(function (o) {
								...
								self.tip.appendDotMouseOver(nextProps.data);
								self.tip.appendBarMouseOver(nextProps.data);
						...
						var yr = parseInt(nextProps.data.match(/\d+/));
...
var StatFilter = React.createClass({
	...
	render: function render() {
		...
		index = this.props.indexDB.findIndex(function (field, index) {
		    ...
		    dataList = this.props.indexDB[index].content.data.map(function (obj) {
			return obj.name;
		}),
		   

		// otherList contains the filters and avialable chart types
		otherList = this.props.indexDB[index].content.data.find(function (obj) {
			if (obj.name === c.data) return true;
		}),
		...
		menus = [datasetList, dataList, otherList.availableChartTypes, otherList.topic];
