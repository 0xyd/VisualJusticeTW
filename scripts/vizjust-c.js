...
'合計發生件數':'#70C1B3','重大竊盜發生件數':'#F25F5C','普通竊盜發生件數':'#FFE066','汽車竊盜發生件數':'#247BA0','機車竊盜發生件數':'#2C5B26','重大竊盜破獲件數':'#9BC53D','重大竊盜尚未破獲件數':'#E55934','重大竊盜嫌疑犯人數':'#F24B48','重大竊盜破獲率':'#EDAFB8','普通竊盜破獲件數':'#5BC0EB','普通竊盜尚未破獲件數':'#FA7921','普通竊盜嫌疑犯人數':'#DB8A62','普通竊盜破獲率':'#E56322','汽機車竊盜案件':'#00A8E8','汽車竊盜破獲件數':'#EF233C','汽車竊盜尚未破獲件數':'#EDF2F4','汽車竊盜破獲率':'#0CCA4A','汽車竊盜嫌疑犯人數':'#F24B48','機車竊盜破獲件數':'#35C3D6','機車竊盜尚未破獲件數':'#FF1654','機車竊盜嫌疑犯人數':'#FFD000','非汽機車竊盜發生件數':'#AD0625', // For 殺人罪 (Prosecution)
...
barGraphClass.prototype.transitStackBarToBar=function(header,mHdrs,yLabel){var self=this; // Grape the data from stack.
...
var isHeaderMerged=self._checkColAccessableInOrigin(data,header), // selected headers for the merged.
// selectedHds = isHeaderMerged ? 
// 	self._avlHeaders(data, mHdrs) : [],
selectedHds=isHeaderMerged?mHdrs:[],mergedData=isHeaderMerged?self._mergedColVal(data,selectedHds):[]; // Collpase the stack bars inside the stack group.
...
self._setLinearYScale(r.mergedData.length>0?r.mergedData:data,r.isHeaderMerged?null:header);self._setYAxis('left',r.mergedData.length>0?r.mergedData:data,r.isHeaderMerged?null:header);self._createYAxis(yLabel);self._createBars(data,header,r.mergedData,false);self._markValOnBar(data,header,r.mergedData);});return p;}; /* Switch from percent stack bar to stack bar. */barGraphClass.prototype.transitPCTSBarToSBar=function(yLabel,intl,extl,isOrigin){var self=this,headers=intl.headers.concat(extl.headers).filter(function(d,i){return d!==null&&d!==undefined;}); // Remove the previous y axis.
...
this.__data__.year=this.parentNode.__data__['民國'];});return new tipClass();}).then(function(tip){tip.appendStackBarMouseOver();resolve();});}});return p;}; // working-spot
...
var shouldMergeCols=this._checkColAccessableInOrigin(rows,dOption); // Get the available headers in specific. 
// var avlHeaders = isdOptionMerged ? this._avlHeaders(rows, mHdrs) : [],
var _mrows=shouldMergeCols?this._mergedColVal(rows,mHdrs):[]; // Set the scale
...
self._createBars(rows,defaultCol,_mrows,true,colorSet);self._markValOnBar(rows,defaultCol,_mrows);resolve({data:rows,pad:self.pad,step:self.step,barWidth:self.barWidth,outPadding:self.outPadding});});});return p;}; // working-spot-3: Rename it with a better name
...
barGraphClass.prototype.update=function(xLabel,yLabel,dOption,colorSet){console.log('colorSet: ',colorSet);var self=this;var p=new Promise(function(resolve,reject){var _bars=self.pad.selectAll('rect'),_txts=self.pad.selectAll('.mark'), // Former x value of bars for text marker transition.
