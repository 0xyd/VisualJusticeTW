...
var DataFilterStateTree = {
	state: Map().set('prosecution', List([{
		dataset: '殺人罪',
		availableChartTypes: ['長條圖', '趨勢圖'],
		content: {
			data: [{
				name: '死刑',
				topics: [['總數'], ['趨勢']]
...
var DataBoard = React.createClass({
	...
	vizDataWithBarChart: function vizDataWithBarChart(props, dataSheet) {
		...
		if (update) {
			bG.update(dataSheet.url, dataSheet.axes.xAxis, dataSheet.axes.yAxis, props.data).then(function () {
	...
	vizDataWithLineChart: function vizDataWithLineChart(props, dataSheet) {
		...
		if (update) {
			lG.update(dataSheet.url, dataSheet.axes.xAxis, dataSheet.axes.yAxis, props.data).then(function () {
	...
	componentWillUpdate: function componentWillUpdate(nextProps, nextStates) {
		...
		var shouldRenew = this.props.dataset !== nextProps.dataset || this.props.chartType !== nextProps.chartType ? true : false,
		    shouldUpdate = this.props.chartType === nextProps.chartType ? true : false;
		...
		if (shouldRenew) {
			...
			} else if (nextProps.chartType === '圓環比例圖') this.vizDataWithRingChart(nextProps, dataSheet, true);
		}
	},
	render: function render() {
...
function selectAppTheme(state, theme) {
	...
	switch (theme) {
		case 'POLICE_STAT':
			...
			return state.merge(navState, mainState, themeState, statTitle, filterNames, defaultDataset, defaultData, defaultChartType, defaultTopic);
		...
		case 'PROSECUTION_STAT':
			...
			defaultFilterDropdownMenus = setState('filterDropdownMenus', List([Map().set('Options', DataFilterStateTree.listDataset('prosecution')).set('isDisplayed', false), Map().set('Options', DataFilterStateTree.listData('prosecution', 0)).set('isDisplayed', false), Map().set('Options', DataFilterStateTree.listCharttype('prosecution', 0)).set('isDisplayed', false), Map().set('Options', DataFilterStateTree.listTopic('prosecution', 0, 0, 0)).set('isDisplayed', false)]));

			return state.merge(navState, mainState, themeState, statTitle, filterNames, defaultDataset, defaultData, defaultChartType, defaultTopic, defaultFilterDropdownMenus);
		...
		case 'CORRECTION_STAT':
			...
			defaultFilterDropdownMenus = setState('filterDropdownMenus', List([
...
function selectDropdownOption(state, theme, optionName, fieldsetIndex, dataIdx) {
	...
	function _FilterRenderDispatcher(state, theme, optionName, fieldsetIndex, datasetIndex) {
		...
		if (fieldsetIndex === 0) {
			...
			if (currentState.get('currentDataset') !== optionName) {
				...
				return state.merge(newState.dataset, newState.data, newState.chartType, newState.topic, newState.dropdownMenuStates);
			}
		...
		// Selecting data
				...
				if (currentState.get('currentData') !== optionName) {
					...
					var newState = __dataSwitchRendering(state, optionName);

					return state.merge(newState.data, newState.dropdownMenuStates);
				}
				return state.merge(collapsedAllDropdownMenuStates);
			}
	...
	function __datasetSwitchRendering(theme, optionName, datasetIndex) {
		...
		var newTopic = setState('currentTopic', stateTree.get(datasetIndex).content.data[0].topics[0]);
