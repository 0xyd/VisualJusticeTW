...
return isDatasetIn?chain.data===dataName&&chain.vizType===vizTypeName:null;}else return chain.dataset===datasetName&&chain.data===dataName&&chain.vizType===vizTypeName;});} // working-spot: Decide which tales chain shoule be applied.
...
else if(endDepth-startDepth<0){for(var s=startDepth-1;s>=endDepth;--s){if(s===startDepth-1){this._vizStory.bwdSteps[s].end=this._vizStory.bwdSteps[s].transit(bwdOpParams[s]._,bwdOpParams[s].params);}else {this._vizStory.bwdSteps[s].end=this._vizStory.bwdSteps[s+1].end.then(this._vizStory.bwdSteps[s].transit.bind(null,bwdOpParams[s]._,bwdOpParams[s].params));}}}}}]);return StoryTeller;}(); /* ***** Elements for the Index Page ***** */var IndexNavList=React.createClass({displayName:'IndexNavList',getInitialState:function getInitialState(){return {nav:[React.createElement(RR.Link,{to:'/about_us'},React.createElement('img',{src:'./src/aboutus.png'})),React.createElement(RR.Link,{to:'/main'},React.createElement('img',{src:'./src/see.png'})),React.createElement(RR.Link,{to:'/special'},React.createElement('img',{src:'./src/issue.png'})),React.createElement(RR.Link,{to:'/work_together'},React.createElement('img',{src:'./src/work.png'})),React.createElement('div',{className:'social-group'},React.createElement('iframe',{id:'githubStar',className:'social-btn',src:'https://ghbtns.com/github-btn.html?user=twbs&repo=bootstrap&type=star&count=true',frameborder:'0',scrolling:'0',width:'170px',height:'20px'}),React.createElement('div',{id:'FBLike',className:'fb-like','data-href':'https://developers.facebook.com/docs/plugins/','data-layout':'button_count','data-action':'like','data-show-faces':'true','data-share':'true'}))]};},render:function render(){var listItems=[],l=this.state.nav.length;for(var i=0;i<l;i++){if(i===l-1){listItems.push(React.createElement(IndexNavListSocialItem,{key:i,elements:this.state.nav[i]}));}else {listItems.push(React.createElement(IndexNavListItem,{key:i,link:this.state.nav[i]}));}}return React.createElement('nav',{id:'NAV',className:'b12-col-md-12 b15-row-md-9'},React.createElement('ul',{className:'b12-col-md-12 b15-row-md-15'},listItems));}});var IndexNavListItem=React.createClass({displayName:'IndexNavListItem',getInitialState:function getInitialState(){return {isHovered:false};},mouseEnterItem:function mouseEnterItem(){this.setState({isHovered:true});},mouseLeaveItem:function mouseLeaveItem(){this.setState({isHovered:false});},render:function render(){return React.createElement('li',{onMouseEnter:this.mouseEnterItem,onMouseLeave:this.mouseLeaveItem,className:'nav-option b12-col-md-12 b12-row-md-2'},React.createElement('span',{className:'ver-helper'}),React.createElement('div',{className:this.state.isHovered?'nav-option-hovermarker active':'nav-option-hovermarker'}),this.props.link);}}); // working-spot
var IndexNavListSocialItem=React.createClass({displayName:'IndexNavListSocialItem',render:function render(){return React.createElement('li',{className:'nav-option b12-col-md-12 b12-row-md-4'},React.createElement('span',{className:'ver-helper'}),this.props.elements);}}); /* Major Themes are displaying on the index page. */var Theme=React.createClass({displayName:'Theme',render:function render(){return React.createElement('div',{id:'',className:'b12-col-md-3 b12-row-md-12 sect-part sect-part--box bd-right '},React.createElement('div',{className:'b12-col-md-12 b12-row-md-8 sect-part-imgwrapper'},React.createElement('img',{className:'sect-part-img',src:this.props.themeImg})),React.createElement('div',{className:'b12-col-md-12 b12-row-md-1 sect-part-btnwrapper'},React.createElement('span',{className:'ver-helper'}),React.createElement(ThemeButton,{path:this.props.path,btnTxtSrc:this.props.btnTxt})),React.createElement('div',{className:'b12-col-md-12 b12-row-md-3 sect-part-imgwrapper'},React.createElement('span',{className:'ver-helper'}),React.createElement('div',{className:'imgtxt-wrapper'},React.createElement('img',{className:'imgtxt',src:this.props.info}))));}});var ThemeBtn=React.createClass({displayName:'ThemeBtn',render:function render(){return React.createElement('div',{className:'sect-part-btn'},React.createElement(RR.Link,{to:this.props.path},React.createElement('img',{className:'sect-part-btn-img',src:this.props.btnTxtSrc})));}}); /* ***** DataBoard components: The components render the visualized data  ***** */var DataBoard=React.createClass({displayName:'DataBoard', /* Customized Methods */ // Graph unit for drawing.
...
var fwd=[];var _iteratorNormalCompletion3=true;var _didIteratorError3=false;var _iteratorError3=undefined;try{for(var _iterator3=fwdTopics[Symbol.iterator](),_step3;!(_iteratorNormalCompletion3=(_step3=_iterator3.next()).done);_iteratorNormalCompletion3=true){var topic=_step3.value;fwd.push({_:this,params:[{dataset:props.dataset,data:props.data,chartType:props.chartType,topic:topic}]});}}catch(err){_didIteratorError3=true;_iteratorError3=err;}finally {try{if(!_iteratorNormalCompletion3&&_iterator3.return){_iterator3.return();}}finally {if(_didIteratorError3){throw _iteratorError3;}}}var bwd=[];var _iteratorNormalCompletion4=true;var _didIteratorError4=false;var _iteratorError4=undefined;try{for(var _iterator4=bwdTopics[Symbol.iterator](),_step4;!(_iteratorNormalCompletion4=(_step4=_iterator4.next()).done);_iteratorNormalCompletion4=true){var topic=_step4.value;bwd.push({_:this,params:[{dataset:props.dataset,data:props.data,chartType:props.chartType,topic:topic}]});}}catch(err){_didIteratorError4=true;_iteratorError4=err;}finally {try{if(!_iteratorNormalCompletion4&&_iterator4.return){_iterator4.return();}}finally {if(_didIteratorError4){throw _iteratorError4;}}}return {fwd:fwd,bwd:bwd};},DBTaleFactory:function DBTaleFactory(){if( // temporary if statement
...
// Select the chain 
this.storyTeller.decideVizStoryChain(nextProps.dataset,nextProps.data,nextProps.chartType); // working-spot
// Select the tales chain
this.storyTeller.decideTaleChain(nextProps.dataset,nextProps.data,nextProps.chartType);return true;} // working-spot
return false;}, // Initial Data Visualizing
...
topicDepth:topicDepth};} /* Action creator for tales */function rollingTalesAC(topic,topicDepth,taleChain,taleIndex){return {type:'INCRE_TALE_IND',topic:topic,topicDepth:topicDepth,taleIndex:taleIndex,taleChain:taleChain};}function back2FirstTaleAC(taleChain){return {type:'BACK_TO_FIRST_TALE',taleChain:taleChain};}function setTaleIndexAC(index){return {type:'SET_TALE_INDEX',taleIndex:index};} // working-spot
function selectTaleAC(index,context){return {type:'SELECT_TALE',taleIndex:index,taleContext:context};} /* ***** Reducers ***** */var INITIAL_STATE=Map();function AppReducer(){var state=arguments.length<=0||arguments[0]===undefined?INITIAL_STATE:arguments[0];var action=arguments[1];switch(action.type){case 'SET_NAV':return setAppNavIndex(state,action.components);case 'SET_MAIN':return setAppMainIndex(state,action.components);case 'SET_THEMES':return setAppMainThemes(state);case 'SELECT_THEME':return selectAppTheme(state,action.themeName);case 'EXPAND_DROPDOWN':return setDropdownMenuStates(state,action.dropdownIndex); // Set the tale index
...
var defaultTopicDepth=null; // working-spot
...
else if(fieldsetIndex===3){if(currentState.get('currentTopic')!==optionName){newTopic=setState('currentTopic',optionName);newTopicDepth=setState('currentTopicDepth',topicDepth); // working-spot: The tale index should be correspond to the topic.
// We have to come up an idea to solve this.
newTaleIndex=setState('currentTaleIndex',0);return state.merge(newTopic,newTopicDepth, // workng-spot
...
var newTopic=setState('currentTopic',newDropdownMenuStates.get('filterDropdownMenus').get(fieldsetIndex+1).get('Options').get(0));return {chartType:newChartType,topic:newTopic, // working-spot
